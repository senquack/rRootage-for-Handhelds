...
static float bgColor[MODE_NUM][3] = {
   {0.0f, 0.0f, 0.0f}, {0.1f, 0.1f, 0.3f}, {0.3f, 0.1f, 0.3f}, {0.1f, 0.3f,
                                                                0.1f},
};
...
void
initStageState (int stg)
{
   float rb;
   int rn, sn;
   int i;
   scene = -1;
   rn = stg / SAME_RANK_STAGE_NUM;
...
drawRPanel ()
{
   ...
   if (left >= 0) {
      ...
      case PSY_MODE:
         ml = ship.grzCnt / 40;
//      drawBox(550, 460, 50, 8, 120, 120, 120);
//      drawBoxx(f2x(550), f2x(460), f2x(50), f2x(8), 120, 120, 120);
         drawBoxx (INT2FNUM (550), INT2FNUM (460), INT2FNUM (50),
                   INT2FNUM (8), 120, 120, 120);
//      drawBox(500+ml, 460, ml, 8, 210, 210, 240);
//      drawBoxx(f2x(500+ml), f2x(460), f2x(ml), f2x(8), 210, 210, 240);
         drawBoxx (INT2FNUM (500 + ml), INT2FNUM (460), INT2FNUM (ml),
                   INT2FNUM (8), 210, 210, 240);
         break;
      case GW_MODE:
         ml = (ship.rfMtr - ship.rfMtrDec) / 40;
//      drawBox(550, 460, 50, 8, 120, 120, 120);
//      drawBoxx(f2x(550), f2x(460), f2x(50), f2x(8), 120, 120, 120);
         drawBoxx (INT2FNUM (550), INT2FNUM (460), INT2FNUM (50),
                   INT2FNUM (8), 120, 120, 120);
//      drawBox(500+ml, 460, ml, 8, 210, 240, 210);
//      drawBoxx(f2x(500+ml), f2x(460), f2x(ml), f2x(8), 210, 240, 210);
         drawBoxx (INT2FNUM (500 + ml), INT2FNUM (460), INT2FNUM (ml),
                   INT2FNUM (8), 210, 240, 210);
         if (ml >= 50) {
            drawString (okStr, 540, 460, 10, 0, 230, 240, 230);
         }
         break;
...
//    drawString(lStr, 40+480, 280, 18, 1, 200, 200, 222);
//    drawLetter(left, 40+480, 420, 18, 1, 230, 180, 150);
//    drawString(lStr, 360, 8, 6, 0, 200, 200, 222);
//    drawLetter(left, 405, 8, 6, 0, 230, 180, 150);
//    drawString(lStr, 185, 45, 9, 0, 200, 200, 222);
//    drawLetter(left, 245, 45, 9, 0, 230, 180, 150);
...
//      drawString(bStr, 90+480, 280, 18, 1, 200, 200, 222);
//      drawLetter(bomb, 90+480, 420, 18, 1, 230, 180, 150);
//      drawString(bStr, 425, 8, 6, 0, 200, 200, 222);
//      drawLetter(bomb, 470, 8, 6, 0, 230, 180, 150);
//      drawString(bStr, 270, 45, 9, 0, 200, 200, 222);
//      drawLetter(bomb, 330, 45, 9, 0, 230, 180, 150);
//      drawString(bStr, 270, 45, 6, 0, 200, 200, 222);
//      drawLetter(bomb, 330, 45, 6, 0, 230, 180, 150);
...
//      drawBox(550, 460, 50, 8, 120, 120, 120);
//      drawBoxx(f2x(550), f2x(460), f2x(50), f2x(8), 120, 120, 120);
//      drawBoxx(f2x(430), f2x(15), f2x(50), f2x(8), 120, 120, 120);
         drawBoxx (INT2FNUM (430), INT2FNUM (15), INT2FNUM (50),
                   INT2FNUM (8), 120, 120, 120);
//      drawBox(500+ml, 460, ml, 8, 210, 210, 240);
//      drawBoxx(f2x(500+ml), f2x(460), f2x(ml), f2x(8), 210, 210, 240);
//      drawBoxx(f2x(380+ml), f2x(15), f2x(ml), f2x(8), 210, 210, 240);
         drawBoxx (INT2FNUM (380 + ml), INT2FNUM (15), INT2FNUM (ml),
                   INT2FNUM (8), 210, 210, 240);
         break;
      case GW_MODE:
         ml = (ship.rfMtr - ship.rfMtrDec) / 40;
...
//      drawBox(500+ml, 460, ml, 8, 210, 240, 210);
//      drawBoxx(f2x(500+ml), f2x(460), f2x(ml), f2x(8), 210, 240, 210);
//      drawBoxx(f2x(380+ml), f2x(15), f2x(ml), f2x(8), 210, 240, 210);
         drawBoxx (INT2FNUM (380 + ml), INT2FNUM (15), INT2FNUM (ml),
                   INT2FNUM (8), 210, 240, 210);
         if (ml >= 50) {
// drawString(okStr, 540, 460, 10, 0, 230, 240, 230);
...
initAttractManager ()
{
   ...
   for (i = 0; i < 12; i++, y += STG_BOX_SIZE * 1.2f) {
      x = 180;
      if (i < 11) {
         for (j = 0; j < SAME_RANK_STAGE_NUM;
              j++, s++, x += STG_BOX_SIZE * 1.2f) {
            stageX[s] = x;
            stageY[s] = y;
         }
      } else {
...
moveTitleMenu ()
{
   ...
   if (pad & PAD_DOWN) {
      ...
      if (mnp) {
         if (slcStg >= 0) {
            if ((slcStg % SAME_RANK_STAGE_NUM) < SAME_RANK_STAGE_NUM - 1
                && slcStg != QUIT_STAGE_NUM) {
               slcStg++;
            }
         } else if (slcStg < -1) {
      ...
      if (mnp) {
         if (slcStg >= 0) {
            if ((slcStg % SAME_RANK_STAGE_NUM) > 0
                && slcStg != QUIT_STAGE_NUM) {
               slcStg--;
            }
         } else if (slcStg > -4) {
   ...
   for (i = -MODE_NUM; i < STAGE_NUM + 1; i++) {
      ...
      if (i == slcStg) {
         ...
         if (titleCnt < 16)
            sz = sz * titleCnt / 16;
//      drawBox(sx, sy, sz, sz, r, g, b);
//      drawBoxx(f2x(sx), f2x(sy), f2x(sz), f2x(sz), r, g, b);
         drawBoxx (INT2FNUM (sx), INT2FNUM (sy), INT2FNUM (sz),
                   INT2FNUM (sz), r, g, b);
         sz = sz * 3 / 5;
         if (i < 0) {
            int md = MODE_NUM + i;
            ...
            drawString (stgChr, 330, 133, 12, 0, 210, 210, 240);
            drawString (stageStr, 445, 133, 12, 0, 210, 210, 240);
            drawNumCenter (hiScore.score[mode][i], 466, 168, 12, 210, 210,
                           240);
         } else {
            drawLetter ('Q' - 'A' + 10, sx, sy, sz, 0, 210, 210, 240);
            drawString (quitChr, 410, 133, 12, 0, 210, 210, 240);
...
//      drawBox(sx, sy, STG_BOX_SIZE/2, STG_BOX_SIZE/2, r*2/3, g*2/3, b*2/3);
//      drawBoxx(f2x(sx), f2x(sy), f2x(STG_BOX_SIZE/2), f2x(STG_BOX_SIZE/2), r*2/3, g*2/3, b*2/3);
         drawBoxx (INT2FNUM (sx), INT2FNUM (sy),
                   INT2FNUM (STG_BOX_SIZE) >> 1,
                   INT2FNUM (STG_BOX_SIZE) >> 1, r * 2 / 3, g * 2 / 3,
                   b * 2 / 3);
      }
   }
   drawString (mdChr[mode], mdChrX[mode], 455, 12, 0, 150, 150, 200);
...
// drawNumCenter(hiScore.score[mode][i], 466, 168, 12, 210, 210, 240);
            ...
            drawNumCenter (hiScore.score[mode][i], 476, 168, 12, 210, 210,
                           240);
         } else {
            drawLetter ('Q' - 'A' + 10, sx, sy, sz, 0, 210, 210, 240);
//senquack - move everything over a tiny bit when screen is rotated:
...
drawBossScoreAtr ()
{
   ...
   if (bsCnt < 64)
      return;
//  drawBox(320, 272, 150, 4, 200, 200, 220);
//  drawBoxx(f2x(320), f2x(272), f2x(150), f2x(4), 200, 200, 220);
   drawBoxx (INT2FNUM (320), INT2FNUM (272), INT2FNUM (150), INT2FNUM (4),
             200, 200, 220);
   if (bsCnt < 96)
      return;
   drawNumCenter (1, 230, 306, 16, 200, 200, 220);
