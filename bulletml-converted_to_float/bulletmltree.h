...
//    DECLSPEC double getValue() const { return val_->value(); }
   DECLSPEC float getValue () const
   {
      return val_->value ();
   }

   DECLSPEC void setType (const std::string & type)
   {
      type_ = string2type (type);
   }
   DECLSPEC Type getType () const
   {
      return type_;
   }

   DECLSPEC void setRefID (int id)
   {
      refID_ = id;
   }
   DECLSPEC int getRefID () const
   {
      return refID_;
   }
