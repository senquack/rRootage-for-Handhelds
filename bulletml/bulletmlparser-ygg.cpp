...
void
BulletMLParserYggdrasil::on_error (yggdrasil::ygg_error * parse_error)
{
...
void
BulletMLParserYggdrasil::start_document (yggdrasil::ygg_node)
{

}

void
BulletMLParserYggdrasil::end_document (yggdrasil::ygg_node &)
{

}

void
BulletMLParserYggdrasil::start_element (yggdrasil::ygg_node element)
{
   BulletMLNode *xmlNode = addContent (trimString (element.get_name ()));

   if (xmlNode->getName () != BulletMLNode::bulletml) {
      MyAttributes mattr;
      for (ygg_iterator ite = element["@*"].begin (); ite.is_not_end ();
           ++ite) {
      ...
      if (curNode_ != 0)
         curNode_->addChild (xmlNode);
   }
   curNode_ = xmlNode;
}

void
BulletMLParserYggdrasil::end_element (yggdrasil::ygg_node element)
{
...
void
BulletMLParserYggdrasil::catch_text (yggdrasil::ygg_node text)
{
   curNode_->setValue (trimString (text.get_text ()));
}

BulletMLParserYggdrasil::BulletMLParserYggdrasil (const std::
                                                  string & filename):
xmlFile_ (filename),
curNode_ (0)
{
