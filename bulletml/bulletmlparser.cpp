...
BulletMLNode *
BulletMLParser::getBulletRef (int id)
{
   BulletMLError::doAssert ((int) bulletMap_.size () > id
                            && bulletMap_[id] != 0,
...
BulletMLNode *
BulletMLParser::getActionRef (int id)
{
   BulletMLError::doAssert ((int) actionMap_.size () > id
                            && actionMap_[id] != 0,
...
BulletMLNode *
BulletMLParser::getFireRef (int id)
{
...
BulletMLNode *
BulletMLParser::addContent (const std::string & name)
{
   // ルートノードは別処理
   if (name == "bulletml") {
...
void
BulletMLParser::addAttribute (const MyAttributes & attr, BulletMLNode * elem)
{
   if (!attr.empty ()) {
