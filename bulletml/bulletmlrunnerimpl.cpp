...
&BulletMLRunnerImpl::runBullet,
      ...
      &BulletMLRunnerImpl::runFireRef, &BulletMLRunnerImpl::runVanish};

//senquack - complete conversion to floats:
//double BulletMLRunnerImpl::getDirection(BulletMLNode* dirNode,
float
BulletMLRunnerImpl::getDirection (BulletMLNode * dirNode, bool prevChange)
{
//senquack - complete conversion to floats:
//    double dir;
   float dir;
...
//    double spd;
float
BulletMLRunnerImpl::getSpeed (BulletMLNode * spdNode)
{
   float spd;
...
BulletMLRunnerImpl::BulletMLRunnerImpl (BulletMLState * state, BulletMLRunner * runner):bulletml_ (state->getBulletML ()),
...
actIte_ (0), end_ (false), runner_ (runner)
{
   ...
   for (std::vector < BulletMLNode * >::iterator ite = node_.begin ();
        ite != node_.end (); ++ite) {
...
float
BulletMLRunnerImpl::getNumberContents (const BulletMLNode * node)
{
   assert (node);
...
void
BulletMLRunnerImpl::setDirection ()
{
...
bool
BulletMLRunnerImpl::isTurnEnd ()
{
   return isEnd () || actTurn_ > endTurn_;
}

void
BulletMLRunnerImpl::doWait (int frame)
{
   if (frame <= 0)
      return;
   actTurn_ += frame;
...
BulletMLRunnerImpl::run ()
{
   ...
   act_ = node_[actIte_];
   if (actTurn_ == -1)
      actTurn_ = runner_->getTurn ();

   runSub ();

   if (act_ == 0) {
      actIte_++;
      if (node_.size () != actIte_)
         act_ = node_[actIte_];
   } else {
      node_[actIte_] = act_;
   }
...
BulletMLRunnerImpl::runFire ()
{
   ...
   if (bullet == 0)
      bullet = act_->getChild (BulletMLNode::bulletRef);
...
BulletMLRunnerImpl::runRepeat ()
{
   ...
   if (action == 0)
      action = act_->getChild (BulletMLNode::actionRef);
   BulletMLError::doAssert (action != 0,
...
BulletMLRunnerImpl::runChangeDirection ()
{
   int term =
      static_cast <
      int >(getNumberContents (act_->getChild (BulletMLNode::term)));
...
void
BulletMLRunnerImpl::calcChangeDirection (float direction, int term, bool seq)
{
...
void
BulletMLRunnerImpl::calcChangeSpeed (float speed, int term)
{
   int finalTurn = actTurn_ + term;
...
void
BulletMLRunnerImpl::calcAccelY (float horizontal, int term,
...
void
BulletMLRunnerImpl::calcAccelX (float vertical, int term,
                                BulletMLNode::Type type)
{
   int finalTurn = actTurn_ + term;
