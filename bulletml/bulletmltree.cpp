...
"bullet", "action", "fire", "changeDirection", "changeSpeed", "accel",
      ...
      "horizontal", "vertical", "term", "times", "direction", "speed",
      "param", "bulletml"};
...
BulletMLNode::Type BulletMLNode::string2type (const std::string & str)
{
   if (str == "aim")
      ...
      BulletMLError::
         doAssert (std::string ("BulletML parser: unknown type ") + str +
                   ".");
...
BulletMLNode::Name BulletMLNode::string2name (const std::string & str)
{
   if (str == "bulletml")
      ...
      BulletMLError::doAssert (std::string ("BulletML parser: unknown tag ") +
                               str + ".");

   return nameSize;             // not reach
}

BulletMLNode::BulletMLNode (const std::string & name):name_ (string2name (name)),
type_ (none)
{
   setReleaseDuty (true);
}
...
BulletMLNode::next ()
{
   ...
   BulletMLError::doAssert (ite != parent->childEnd (),
                            name_ + ": not found");
   ite++;
