...
template < typename Val_ > class AbstractNumber {
 public:
...
template < typename Val_ > class Number:public AbstractNumber < Val_ > {
 public:
...
typedef enum
{ op_null = 0, op_add, op_sub, op_mul, op_div } Operator;

template < typename Val_ > class Formula:public AbstractNumber < Val_ > {
 private:
 ...
 private:
     Val_ valueBeforeHeadSub () const
   {
      ...
      case op_null:
         return lhs_->value ();
         case op_add:return lhs_->value () + rhs_->value ();
         case op_sub:return lhs_->value () - rhs_->value ();
         case op_mul:return lhs_->value () * rhs_->value ();
         case op_div:return lhs_->value () / rhs_->value ();
         default:return 0;      // avoid warning
      }
   }
