...
//  ygg_string_replace(X_text, "\t", "&#9;");
      ...
      ygg_string ygg_utility::encode_sox (const ygg_string & indent,
                                           const ygg_string & X) {
         ygg_string X_text;
         ygg_string buffer = X;
         ygg_string::size_type p;
         const int return_code_length = ygg_term::return_code.length ();
         while (ygg_string::npos !=
...
#	endif //  __ACCEPT_NULLCODE__
            ...
            operator  const char *()
            {
               ...
               || (X_replace = X_text.substr (p, p_end + 1), true);
...
//  indent;
            ...
            if (base_encoding::sjis == get_base_encoding ()) {
               ...
               for (ygg_string::const_iterator i = X.begin (); i != X.end ();
                  ...
                  if (i == X.end () || 0x40 <= (unsigned char) *i
            ...
            return no_error;
                                                         ...
                                                         const ygg_string &
                                                         X_replace) {
          for (ygg_string::size_type p = ygg_string::npos, search_length =
                 X_search.length ();
      ...
       ygg_string ygg_utility::create_line (ygg_node X_list,
   ...
   ygg_string ygg_node::get_path ()const
   {
      if (body.is_null ())
      {
             ...
             || type == ygg_node_type::attribute) {
               ...
               pre_node.set_value (pre_node.get_value ().get_string () +
...
//  return indent +value +ygg_term::comment_trail +ygg_term::return_code;
    ...
    ygg_string ygg_proto_list::get_text () const
   {
      ...
      for (ygg_list_type::const_iterator i = body.begin (); i != body.end ();
            ++i)
      {
...
//  return ygg_term::list_node;
    ...
    ygg_node ygg_root::operator[] (const ygg_string & path)
   {
         ...
         for (ygg_list_type::iterator i = body.begin (); i != body.end ();
               ++i) {
         ...
         return operator[](ygg_term::path_root).enum_node (path.
                                                            substr (ygg_term::
                                                                    path_wherever.
                                                                    length
                                                                    ()));
            ...
            return get_shell ().enum_node (path.
                                            substr (ygg_term::
                                                    path_this__wherever.
                                                    length ()));
...
//  }
    ...
    ygg_string ygg_element::get_sox (const ygg_string & indent) const
   {
       ...
       for (ygg_list_type::const_iterator i = X_list.begin ();
              i != X_list.end (); ++i)
      {
   ...
    ygg_node ygg_element::clone ()const
   {
      ...
      const ygg_string::size_type return_code_length =
   ...
    const ygg_error_code ygg_parser::check_name (const ygg_string & type,
                                                   const ygg_string & X)
   {
...
//      hot_element = hot_element.get_parent();
         ...
         last_node.set_value (last_node.get_value ().get_string () +
                               ygg_term::
                               return_code  +make_indent (indent_count -
                                                           (anchor_indent +
                                                            1))  +X_parse);
            ...
            raise_error (ygg_error_term::duplication_attribute,
                          name + "属性が重複しています。");
...
//
      xml_parser & xml_parser::init_root () {
         ...
         if (parse_error.is_error ()) {
               ...
               PROMOTION (unparsed_buffer).substr (p_end +
               ...
               ygg_string::size_type p_trail_sign =
...
#				endif
                       ...
                       && p_lead_sign == element_string.find ("?",
                                                               p_lead_sign)
                       && p_trail_sign == element_string.find ("?",
                                                                p_trail_sign))
               {
...
#				endif
                     ...
                     ygg_string attribute_name =
                        element_string.substr (0, p_name_end);
                     if (ygg_parser::check_attribute_name (attribute_name).
                          is_error ()) {
                     ...
                     ygg_string::size_type p_value_a =
                        element_string.find ("\"", p_name_end);
                     ygg_string::size_type p_value_b =
                        element_string.find ("\'", p_name_end);
                     if (ygg_string::npos == p_value_b
                          || (ygg_string::npos != p_value_a
                               && p_value_a <= p_value_b)) {
                  ...
                  if (lead_sign || trail_sign) {
                        ...
                        raise_error (ygg_error_term::unmatch_tags_a,
                                      "開始タグ<" + hot_element.get_name () +
                                      ">と終了タグ</" + element_name +
                                      ">がマッチしていません。");
                     } else {
                        raise_error (ygg_error_term::unmatch_tags_b,
                                      "いきなり終了タグ<" + element_name +
                                      ">から始まっています。");
