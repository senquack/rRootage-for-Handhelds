...
//  const ygg_string root_node = "root()";
      const ygg_string path_dir = "/";
         ...
         " !\"#$%&'()*+,/;<=>?@[\\]^`{|}~";
       const ygg_string yggdrasil_version =
         "information>\n"  "\tmodule>\tid=yggdrasil\n" 
         "\t\tname=世界樹  -yggdrasil- \n" 
...
//
      ...
      ygg_string encode_sox (const ygg_string & indent,
                              const ygg_string & X);
      ...
       ygg_node & format_tree (ygg_node & node,
                                 unsigned int max_row_length = 40);
                                          ...
                                          const ygg_string & search,
                                          const ygg_string & replace);
       ygg_string create_line (ygg_node X_list,
...
//  ygg_node_body *body;
      ...
      inline ygg_string get_xml (const ygg_string & indent =
                                  ygg_term::empty) const;
      inline ygg_string get_xml_attribute ()const;
      inline ygg_string get_sox (const ygg_string & inden =
                                  ygg_term::empty) const;
      ...
      inline ygg_node & adopt_sox (const ygg_string & sox,
                                    sax_handler * sax = NULL) {
         return adopt_node (parse_sox (sox, sax));
      }
      inline ygg_node & adopt_xml (const ygg_string & xml,
                                    sax_handler * sax = NULL) {
...
//      }
      ...
       static inline ygg_node parse_sox (const ygg_string & sox,
                                           sax_handler * sax = NULL);
      static inline ygg_node parse_xml (const ygg_string & xml,
                                         sax_handler * sax = NULL);
...
#endif  /*  */
    public:static ygg_node create (ygg_root * X_parent,
                                const ygg_string & X_path)
      {
...
//typedef std::bidirectional_iterator<ygg_node, ptrdiff_t> ygg_iterator_base;
      struct ygg_iterator_base
   {
    ...
     public:ygg_const_iterator (const ygg_node X_list,
                           ygg_list_type::
...
//
 class ygg_root:public ygg_proto_list {
   ...
    inline ygg_node ygg_node::parse_sox (const ygg_string & sox,
                                           sax_handler * sax)
   {
...
//  node.self_exile();
   ...
   inline ygg_node ygg_node::parse_xml (const ygg_string & xml,
                                         sax_handler * sax) {
...
//
    ...
    } ygg_error_code (const ygg_error_code & X, int X_case = 0):value (X.value +
             X_case)
      {
...
//
      ...
      inline const ygg_error_code make_category (const ygg_error_code &
                                                  X_genre, int X_category)
      {
         return X_genre.get_code () + X_category * category;
      }
      inline const ygg_error_code make_code (const ygg_error_code &
                                              X_category, int X_sequence,
                                              int X_sequence_case = 0) {
         return X_category.get_code () + X_sequence * sequence +
            X_sequence_case * sequence_case;
      }
      inline const ygg_error_code make_strict_code (const ygg_error_code &
                                                     X_sequence,
                                                     int X_sequence_case)
      {
      ...
      const ygg_error_code ygg_invalid_name_a =
         make_strict_code (ygg_invalid_name, 0x01);
      const ygg_error_code ygg_invalid_name_b =
      ...
      const ygg_error_code unmatch_tags_a =
         make_strict_code (unmatch_tags, 0x01);
      const ygg_error_code unmatch_tags_b =
...
//
    ...
     ygg_parser (sax_handler * X_sax = NULL):anchor_position (), hot_position (),
         sax (X_sax) {
         if (sax) {
            sax->set_parser (this);
         }
      }
       const ygg_error_code check_name (const ygg_string & type,
                                          const ygg_string & X);
       ...
       const int get_row () const
      {
       ...
       void raise_error (const ygg_error_code & X_code =
...
//
    ...
    public:virtual void on_error (ygg_error * parse_error);
                                                        //  parse_error->clear() 可
...
//
 class sox_parser:public ygg_parser {
         ...
         root (X_root)
      {
      } sox_parser & set_sax_handler (sax_handler * X_sax) {
      ...
      sox_parser & write (std::ostream & stream, int encoding =
                           babel::base_encoding::unknown);
      sox_parser & load (const ygg_string & X_filename, int encoding =
                          babel::base_encoding::unknown);
      sox_parser & save (const ygg_string & X_filename, int encoding =
                          babel::base_encoding::unknown);
                                            ...
                                            int encoding)
   {
      ...
      inline sox_parser & sox_parser::read (std::istream & stream) {
         ...
         init_root ();
         ...
         raise_error (ygg_error_term::invalid_stream,
                       "無効なストリームが指定されました。");
            ...
             const int determine_encoding =
               (babel::base_encoding::unknown ==
                encoding) ? analyze_base_encoding (second_buffer) : encoding;
            bbl_translater < bbl_binary, ygg_string > translater =
...
#endif //   defined(__YGGDRASIL_WITH_BABEL__)
         if (!file) {
         raise_error (ygg_error_term::cannot_open_file,
                       "ファイル " + X_filename + " を開けませんでした。");
...
//
 class sox_file:public ygg_node {
   ...
   inline babel::bbl_translater < babel::bbl_binary,
...
//
 class xml_parser:public ygg_parser {
         ...
         root (X_root)
      {
      } xml_parser & set_sax_handler (sax_handler * X_sax) {
         set_sax_handler (X_sax);
         return *this;
      }
...
//
 class xml_file:public ygg_node {
   ...
     inline ygg_node ygg_root::parse_sox (const ygg_string & sox,
                                             sax_handler * sax) {
      ygg_node root;
      sox_parser (root, sax).init_root ().parse_string (sox).end_stream ();
      return root;
   }
   inline ygg_node ygg_root::parse_xml (const ygg_string & xml,
                                         sax_handler * sax) {
