...
//      printf("path:%s\n", i->get_path().c_str());
//  }
      std_for (ygg_iterator i = node_list.begin ();
                i.is_not_end ();
                ++i) {
...
//  }
      std_for (ygg_iterator i = element["@*"].begin ();
                i.is_not_end ();
                ++i) {
...
test_const ()
{
   const ygg_node node_list =
      sox_file ("test.sox").read (&vitalize (error_print ()))["//node()"];
   std_for (ygg_const_iterator i = node_list.begin ();
             i.is_not_end ();
             ++i) {
...
//  std::locale::global(std::locale("japanese"));
      ...
      babel::init_babel ();
         ...
         std::cout << ygg_node::parse_sox (ygg_term::yggdrasil_version).
