...
//  fr->y = y / FIELD_SCREEN_RATIO;
//  fr->z = z / FIELD_SCREEN_RATIO;
//  fr->fx = f2x((float)x / FIELD_SCREEN_RATIO);
//  fr->fy = f2x((float)y / FIELD_SCREEN_RATIO);
//  fr->fz = f2x((float)z / FIELD_SCREEN_RATIO);
...
//  fr->fwidth = f2x((float)width / FIELD_SCREEN_RATIO);
...
//}
void
addBossFragx (GLfixed x, GLfixed y, GLfixed z, GLfixed width, int d)
{
//  addLineFragFloat(x, y, z, randNS(1024), randNS(1024), randNS(1024),
//       width, d, 2, 52+randN(20));
...
//}
void
addShipFragx (GLfixed x, GLfixed y)
{
   int i, d, s;
   for (i = 0; i < 80; i++) {
      d = randN (1024);
...
////  cx = (ox*sctbl[d+256] - oy*sctbl[d]) / 256.0f + x;
////  cy = (ox*sctbl[d] + oy*sctbl[d+256]) / 256.0f + y;
//  cx = INT2FNUM((ox*sctbl[d+256] - oy*sctbl[d])>>8) + x;
//  cy = INT2FNUM((ox*sctbl[d] + oy*sctbl[d+256])>>8) + y;
////  ld = (getDeg((int)((ox1-ox2)*256), (int)((oy1-oy2)*256)) + d) & 1023;
//  ld = (getDeg(((ox1-ox2)<<8), ((oy1-oy2)<<8)) + d) & 1023;
////  lw = getDistanceFloat(ox2-ox1, oy2-oy1)/2;
//  lw = INT2FNUM(getDistance(ox2-ox1, oy2-oy1)>>1);
////  addLineFragFloat(cx, cy, 0, mx, -my, -randN(1024)-1024, lw, ld, 4, 24+randN(16));
//  addLineFragx(cx, cy, 0, mx, -my, -randN(1024)-1024, lw, ld, 4, 24+randN(16));
//}
void
addLineFragOfs (GLfixed x, GLfixed y, GLfixed ox1, GLfixed oy1, GLfixed ox2,
                GLfixed oy2, int d, int mx, int my)
{
   GLfixed cx, cy, ox, oy, lw;
   int ld;
//  ox = (ox1+ox2)/2; oy = (oy1+oy2)/2;
...
//  cy = (ox*sctbl[d] + oy*sctbl[d+256]) / 256.0f + y;
   cx =
      INT2FNUM ((FNUM2INT (ox) * sctbl[d + 256] -
                 FNUM2INT (oy) * sctbl[d]) >> 8) + x;
   cy =
      INT2FNUM ((FNUM2INT (ox) * sctbl[d] +
                 FNUM2INT (oy) * sctbl[d + 256]) >> 8) + y;
//  ld = (getDeg((int)((ox1-ox2)*256), (int)((oy1-oy2)*256)) + d) & 1023;
   ld =
      (getDeg (FNUM2INT ((ox1 - ox2) << 8), FNUM2INT ((oy1 - oy2) << 8)) +
       d) & 1023;
////  lw = getDistanceFloat(ox2-ox1, oy2-oy1)/2;
   lw = getDistance (ox2 - ox1, oy2 - oy1) >> 1;

//  addLineFragFloat(cx, cy, 0, mx, -my, -randN(1024)-1024, lw, ld, 4, 24+randN(16));
   addLineFragx (cx, cy, 0, mx, -my, -randN (1024) - 1024, lw, ld, 4,
                 24 + randN (16));
}
...
//}
void
addShapeFragx (GLfixed x, GLfixed y, GLfixed size, int d, int cnt, int type,
               int mx, int my)
{
   int sd;
   GLfixed sz, sz2;
...
//     fr->r[c], fr->g[c], fr->b[c], 255, fr->d1, fr->d2);
      ...
      drawRollLinex (fr->fx, fr->fy, fr->fz, fr->fwidth,
